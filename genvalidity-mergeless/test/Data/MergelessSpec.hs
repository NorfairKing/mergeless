...
spec = do
  ...
  describe "emptyStore" $ it "is valid" $ shouldBeValid (emptyStore @Int @Int)
  describe "storeSize" $ it "does not crash" $ producesValidsOnValids (storeSize @Int @Int)
  describe "addItemToStore" $
    it "produces valid stores" $ producesValidsOnValids2 (addItemToStore @Int @Int)
  describe "deleteUnsynced" $
    it "produces valid stores" $ producesValidsOnValids2 (deleteUnsynced @Int @Int)
  describe "deleteSynced" $
    it "produces valid stores" $ producesValidsOnValids2 (deleteSynced @Int @Int)
  describe "storeSize" $ do
    specify "deleting an unsynced item after adding it leaves the store with the original size" $
      forAllValid $ \store ->
        forAllValid $ \added ->
          let size1 = storeSize (store :: Store Int Int)
    ...
    specify "deleting a synced item after adding it leaves the store with the original size" $
      forAllValid $ \store ->
        forAllValid $ \synced ->
          let size1 = storeSize (store :: Store Int Int)
              store' = Store $ S.insert (SyncedItem synced) $ storeItems store
  ...
  describe "processSyncWith" $ do
    it
      ...
      forAllValid $ \synct ->
        forAllValid $ \sis -> do
          ...
          let (sr, cs') =
                evalD $
                processSyncWith @(UUID Int) @Int genD synct cs $
    ...
    it "deletes the deleted items" $
      forAllValid $ \synct ->
        forAllValid $ \cs ->
          forAllValid $ \sreq -> do
            let (_, cs') = evalD $ processSyncWith @(UUID Int) @Int genD synct cs sreq
            syncRequestUndeletedItems sreq `shouldSatisfy`
              (not . any (`S.member` (M.keysSet $ centralStoreItems cs')))
    it "returns the items that were added in the sync response" $
      forAllValid $ \synct ->
        forAllValid $ \cs ->
          forAllValid $ \sreq -> do
            let (sresp, _) = evalD $ processSyncWith @(UUID Int) @Int genD synct cs sreq
      ...
      forAllValid $ \synct ->
        forAllValid $ \cs ->
          forAllValid $ \ai -> do
            ...
            S.map syncedValue (syncResponseAddedItems sresp) `shouldBe` S.singleton (addedValue ai)
        ...
        forAllValid $ \cs ->
          forAllValid $ \sreq -> do
            ...
            S.map addedValue (syncRequestAddedItems sreq) `shouldSatisfy`
              all (`elem` (M.elems $ M.map centralValue $ centralStoreItems cs'))
    it
        ...
        forAllValid $ \cs ->
          forAllValid $ \sis ->
            forAllValid $ \dis -> do
              ...
              S.map syncedUuid (syncResponseNewRemoteItems sresp) `shouldBe`
                S.difference (S.difference (M.keysSet $ centralStoreItems cs) dis) sis
      ...
      forAllValid $ \synct ->
        forAllValid $ \cs ->
          forAllValid $ \sreq -> do
            ...
            S.map syncedUuid (syncResponseNewRemoteItems sresp) `shouldBe`
              S.difference
                (S.difference (M.keysSet $ centralStoreItems cs) (syncRequestUndeletedItems sreq))
        ...
        forAllValid $ \(synct1, synct2, synct3) -> do
          let (s1, s2) =
                evalI $ do
                  ...
                  (sresp1, central') <- processSyncWith genI synct1 central sreq1
                  let store1' = mergeSyncResponse store1 sresp1
                  let sreq2 = makeSyncRequest store2
                  (sresp2, central'') <- processSyncWith genI synct2 central' sreq2
                  let store2' = mergeSyncResponse store2 sresp2
                  let sreq3 = makeSyncRequest store1'
                  (sresp3, _) <- processSyncWith genI synct3 central'' sreq3
        ...
        forAllValid $ \(synct1, synct2, synct3) -> do
          let (s1, s2) =
                evalD $ do
                  let central = CentralStore M.empty
                  let store2 = Store S.empty
                  let sreq1 = makeSyncRequest @(UUID Int) @Int store1
                  (sresp1, central') <- processSyncWith genD synct1 central sreq1
                  let store1' = mergeSyncResponse store1 sresp1
                  let sreq2 = makeSyncRequest store2
                  (sresp2, central'') <- processSyncWith genD synct2 central' sreq2
                  let store2' = mergeSyncResponse store2 sresp2
                  let sreq3 = makeSyncRequest store1'
                  (sresp3, _) <- processSyncWith genD synct3 central'' sreq3
                  let store1'' = mergeSyncResponse store1' sresp3
                  pure (store1'', store2')
          s1 `shouldBe` s2
    it
      ...
      forAll (genListOf $ (,) <$> genUnsyncedStore <*> genValid) $ \tups ->
        ...
        evalI $ do
          ...
          let go cs (store, synct) = do
                ...
                pure central'
          foldM go initCentralStore (tups :: [(Store Word Int, UTCTime)])
    it
      ...
      forAllValid $ \tups ->
        ...
        evalD $ do
          let initCentralStore = CentralStore M.empty
          let go cs (store, synct) = do
                let sreq = makeSyncRequest @(UUID Int) @Int store
        ...
        forAll (genValid `suchThat` (>= synct1)) $ \synct2 ->
          forAllValid $ \central1 ->
            forAllValid $ \local1 -> do
              ...
              let ((sresp1, central2), d2) = runI (processSyncWith genI synct1 central1 sreq1) d1
              let local2 = mergeSyncResponse local1 sresp1
              let sreq2 = makeSyncRequest local2
              let ((sresp2, central3), _) = runI (processSyncWith genI synct2 central2 sreq2) d2
      ...
      forAllValid $ \synct1 ->
        forAll (genValid `suchThat` (>= synct1)) $ \synct2 ->
          forAllValid $ \central1 ->
            forAllValid $ \local1 -> do
              let d1 = mkStdGen 42
              let sreq1 = makeSyncRequest @(UUID Int) @Int local1
              let ((sresp1, central2), d2) = runD (processSyncWith genD synct1 central1 sreq1) d1
              let local2 = mergeSyncResponse local1 sresp1
              let sreq2 = makeSyncRequest local2
              let ((sresp2, central3), _) = runD (processSyncWith genD synct2 central2 sreq2) d2
              let local3 = mergeSyncResponse local2 sresp2
              local2 `shouldBe` local3
              central2 `shouldBe` central3
      ...
      forAllValid $ \synct1 ->
        forAll (genValid `suchThat` (>= synct1)) $ \synct2 ->
          forAllValid $ \central1 ->
            forAllValid $ \local1 -> do
              let sreq1 = makeSyncRequest @(UUID Int) @Int local1
              (sresp1, central2) <- processSyncWith nextRandomUUID synct1 central1 sreq1
              let local2 = mergeSyncResponse local1 sresp1
              let sreq2 = makeSyncRequest local2
              (sresp2, central3) <- processSyncWith nextRandomUUID synct2 central2 sreq2
              let local3 = mergeSyncResponse local2 sresp2
              local2 `shouldBe` local3
              central2 `shouldBe` central3
